...
export default function Flights() {
  ...
  const cancelFlight = (flight_id: number): void => {
    axios
      .get("https://localhost:8000/cancel_flight", {
  ...
  const loadOptions = (
    ...
    setTimeout(() => {
      axios
        .get<GetAirportCodeResponse>("https://localhost:8000/airports", {
          params: { code: inputValue },
        })
        .then((response) => {
          return callback(
            response.data.data.map((r) => ({ label: r, value: r }))
          );
        })
        .then((response) => {
          return callback(
            response.data.data.map((r: any) => ({ label: r, value: r }))
          );
        });
    }, 1000);
  };
  ...
  useEffect(() => {
    ...
    axios
      .get<FlightType[]>("https://localhost:8000/flights", { params })
      .then((response) => {
        ...
        <div className="mx-auto max-w-3xl py-6 sm:px-6 lg:px-8">
          <div className="border-b border-gray-900/10 pb-12">
            ...
            <p className="mt-1 text-sm leading-6 text-gray-600">
              Enter the IATA codes for the origin and destination airports
            </p>
            <button
              type="button"
              ...
              <div className="inline-flex items-center">
                ...
                >
                  <input
                    ...
                    id="checkbox-8"
                    onChange={() => {
                      setShowOnlyCancelled(!showOnlyCancelled);
                    }}
                  />
          ...
          <div className="grid grid-cols-1 gap-4 justify-center">
            {flights.map((f) => {
              const color =
                f.status && f.status === "canceled"
                  ? "bg-red-400"
                  : "bg-gray-200";
              const status =
                f.status && f.status === "canceled" ? "Cancelled" : "Scheduled";
              const className = `col-start-1 col-end-2 ${color} rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2`;
              return (
                <div
                  key={f.id}
                  className="max-w-full rounded overflow-hidden shadow-lg bg-white"
                >
                  <div className="px-6 py-4">
                    <div className="flex font-bold text-xl">
                      {f.airline_name} #{f.flight_number}:
                      <a href="#" className="ml-5 text-neutral-600">
                        {f.origin_ap_code}
                      </a>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                  ...
                  <div className="w-auto mx-5 rounded bg-slate-100">
                    <div
                      className="bg-primary p-0.5 text-center text-xs font-medium leading-none text-primary-100 bg-green-500"
                      style={{ width: "25%" }}
                    >
                      25%
                    </div>
                  ...
                  <div className="grid grid-cols-6 gap-4 px-6 pt-4 pb-2">
                    ...
                    <button
                      ...
                      className={`col-end-7 col-span-1 rounded ${
                        f.status === "canceled" ? "bg-gray-400" : "bg-gray-800"
                      } px-6 pt-2.5 pb-2 text-xs font-medium uppercase leading-normal text-white shadow-[0_4px_9px_-4px_#3b71ca]`}
                      onClick={() => {
                        cancelFlight(f.id);
                      }}
                    >
